...
def get_clip_score(score_image, score_gt):
    """
    Compute cosine similarity between two images using CLIP's vision encoder.
...
def find_file_by_subject(subject_name, folder):
    """Find file in folder that matches subject_name (ignoring extension)."""
...
for version in range(len(image_prefix)):
    ...
    try:
        for i, subj_name in enumerate(rnd3_subjects):
            # gt_name = ground_truth[i]  # careful: assumes same order!
