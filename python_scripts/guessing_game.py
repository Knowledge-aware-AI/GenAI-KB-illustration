...
my_api_key = os.environ.get("API_KEY")    # make sure have an environmental variable called "API_KEY" with the API key as value exists
client_tu = OpenAI(base_url="https://llm.scads.ai/v1",api_key=my_api_key)

# Find model with "llama-4" in name; if Llama 4 is unresponsive, Qwen2 can be used instead with no major performance changes
...
########### Setting all necessary subject names, can be replaced if other subjects are used ###########
chosen_subjects = [                                        # The handpicked subjects referred to as Regular dataset
...
file_path = "sampled_subjects.txt"    # A txt-file with just the randomly chosen subjects (Random dataset)
...
# Path to the saved txt file
file_path1 = "new/sampled_subjects.txt"     # A txt-file with just the new randomly chosen subjects (New Random dataset)
...
file_path3 = "scale_test/entries_500_nosubjects.pkl"        # A pickle-file with the chosen subjects for the scale test (Random dataset)
...
############ Setup for the loop, make sure all sets are filled with the correct data ############
subjects = [SUBJECTS_1,SUBJECTS_2]    # the types of subjects to loop through
image_path = [r"IMAGE/PATH_1",r"IMAGE/PATH_2"]   # path to the images to have their subjects guessed
gen_variant = ["Experiment 1","Experiment 2"]  # names to be shown in the log to check status of guessing game
...
########## Category Setup - has to be varied for different data ###########
chosen_categories = [
