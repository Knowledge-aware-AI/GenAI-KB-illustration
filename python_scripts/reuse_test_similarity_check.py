...
os.environ["HF_HOME"] = r"PATH/FOR/HUGGINGFACE" # not necessary if enough space on standard save path for the used models
...
def extract_entry(subject_uri):
    ...
    full_entry = list(cur.execute(
        "SELECT predicate, object FROM gptkb WHERE subject = ?",
        (subject_uri,)
    ))
    ...
    if not full_entry:
        ...
        full_entry = list(cur.execute(
            "SELECT predicate, object FROM gptkb WHERE subject LIKE ?",
            (subject_uri_like,)
        ))
...
model = SentenceTransformer("mixedbread-ai/mxbai-embed-large-v1")  # Qwen would be larger, but needs substantially more memory; mixedbread had sufficient size for the usecase
